...
export default function MasterData({ user }) {
  ...
  const tabs = [
    ...
    { id: 'activities', label: '⚙️ Activity Types', table: 'activity_types' },
    { id: 'stages', label: '📊 Activity Stages', table: 'activity_stages' },
  ...
  const fetchData = async () => {
    ...
    if (activeTab === 'workers') {
      ...
      query = supabase.from('activity_stages').select('*').order('sequence_order')
    }
  ...
  const handleDelete = async (id) => {
    ...
    if (activeTab === 'vendors') {
      ...
      if (workers && workers.length > 0) {
        alert('❌ Tidak bisa hapus!\n\nVendor ini masih punya workers. Hapus workers terlebih dahulu.')
        return
      }
      ...
      if (assignments && assignments.length > 0) {
        alert('❌ Tidak bisa hapus!\n\nVendor ini masih punya assignments. Hapus assignments terlebih dahulu.')
        return
      }
    }
    ...
    if (activeTab === 'sections') {
      ...
      if (sectionActivities && sectionActivities.length > 0) {
        alert('❌ Tidak bisa hapus!\n\nSection ini masih punya activity assignments. Hapus assignments terlebih dahulu.')
        return
      }
    }
    ...
    if (activeTab === 'activities') {
      ...
      if (sectionActivities && sectionActivities.length > 0) {
        alert('❌ Tidak bisa hapus!\n\nActivity ini masih digunakan di assignments. Hapus assignments terlebih dahulu.')
        return
      }
    }
    ...
    if (activeTab === 'materials') {
      ...
      if (activityMaterials && activityMaterials.length > 0) {
        alert('❌ Tidak bisa hapus!\n\nMaterial ini masih digunakan di SOP. Hapus SOP terlebih dahulu.')
        return
      }
    }
    ...
    if (activeTab === 'stages') {
      ...
      if (activityMaterials && activityMaterials.length > 0) {
        alert('❌ Tidak bisa hapus!\n\nStage ini masih digunakan di SOP material. Hapus SOP terlebih dahulu.')
        return
      }
    }
  ...
  const getEmptyForm = () => {
    switch (activeTab) {
      ...
      case 'stages':
        return { code: '', name: '', description: '', sequence_order: 0, active: true }
      case 'vendors':
  ...
  const renderForm = () => {
    switch (activeTab) {
      case 'blocks':
        return (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium mb-1">Luas Total (Ha) *</label>
              ...
              <div>
                ...
                <select value={formData.kategori || 'PC'} onChange={(e) => setFormData({...formData, kategori: e.target.value})} className="w-full px-3 py-2 border rounded">
                  <option value="PC">Plant Cane (PC)</option>
                  <option value="RC">Ratoon Cane (RC)</option>
                </select>
              </div>
            </div>
            <div>
        ...
        return (
          <div className="space-y-4">
            <div className="bg-blue-50 border border-blue-200 rounded p-3 text-sm text-blue-800">
              <strong>ℹ️ Info:</strong> Activity Types adalah jenis aktivitas yang bisa dilakukan (contoh: Herbisida, Pupuk, Panen). Assignment ke section dilakukan di menu Assignment Management.
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium mb-1">Code *</label>
            ...
            <div className="space-y-2">
              <label className="flex items-center gap-2">
                <input type="checkbox" checked={formData.requires_material || false} onChange={(e) => setFormData({...formData, requires_material: e.target.checked})} className="w-4 h-4" />
                <span className="text-sm">Memerlukan Material</span>
              </label>
              <label className="flex items-center gap-2">
                <input type="checkbox" checked={formData.requires_vendor || false} onChange={(e) => setFormData({...formData, requires_vendor: e.target.checked})} className="w-4 h-4" />
                <span className="text-sm">Memerlukan Vendor</span>
              </label>
            </div>
          </div>
        )
        ...
        return (
          <div className="space-y-4">
            <div className="bg-green-50 border border-green-200 rounded p-3 text-sm text-green-800">
              <strong>ℹ️ Info:</strong> Activity Stages adalah tahapan/metode dalam suatu activity (contoh: Normal A, Normal B, Alternative 1). Digunakan untuk material SOP yang berbeda per stage.
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium mb-1">Code *</label>
              ...
              <div>
                ...
                <input type="number" value={formData.sequence_order || 0} onChange={(e) => setFormData({...formData, sequence_order: parseInt(e.target.value)})} className="w-full px-3 py-2 border rounded" required />
              </div>
            </div>
            <div>
      ...
      default:
        return <div>Form untuk {activeTab}</div>
    }
  }
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {data.map(item => (
                <tr key={item.id}>
                  ...
                  <td className="px-4 py-3 text-sm">{item.luas_total} Ha</td>
                  <td className="px-4 py-3 text-sm">
                    <span className={`px-2 py-1 rounded text-xs ${item.kategori === 'PC' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'}`}>
                      {item.kategori}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-sm">
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Code</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Nama Stage</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {data.map(item => (
                <tr key={item.id}>
                  <td className="px-4 py-3 text-sm font-medium">{item.sequence_order}</td>
                  <td className="px-4 py-3 text-sm font-medium">{item.code}</td>
                  <td className="px-4 py-3 text-sm">{item.name}</td>
        ...
        <div className="p-6">
          <div className="flex justify-between items-center mb-4">
            <div className="flex gap-2">
              <button onClick={handleNew} className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
                + Tambah
              </button>
              ...
              <button onClick={handleExportExcel} className="px-4 py-2 bg-purple-600 text-white rounded hover:bg-purple-700">
                📤 Export Excel
              </button>
            </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <h2 className="text-xl font-bold mb-4">
              {editData ? 'Edit' : 'Tambah'} {tabs.find(t => t.id === activeTab)?.label}
            </h2>
            {renderForm()}
